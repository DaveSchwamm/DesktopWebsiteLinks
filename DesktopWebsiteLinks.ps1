...
Set-Content -Path "$($ENV:PUBLIC)\Pictures\YOURICONHERE.ico" -Value $ICOContent -Encoding Byte

# Set the icon file to read-only in order to prevent removal/changes without Local Administrator rights
Set-ItemProperty -Path "$($ENV:PUBLIC)\Pictures\YOURICONHERE.ico" -Name IsReadOnly -Value $true
...
$Shortcut = $WshShell.CreateShortcut("$($ENV:PUBLIC)\Desktop\YOURLINKHERE.lnk")
$Shortcut.TargetPath = 'https://your.tld/target/here/'
$Shortcut.IconLocation = "$($ENV:PUBLIC)\Pictures\YOURLINKHERE.ico"
$Shortcut.Save()